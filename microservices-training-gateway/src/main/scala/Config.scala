...
object Config {
  ...
  private val configFile = Try(io.Source.fromFile("/etc/microservicestraininggateway/microservicestraininggateway.json").mkString) getOrElse "{}"
  ...
  val host = environment match {
    ...
    case "int" | "test" => s"https://microservicestraininggateway.$environment.api.bbci.co.uk"
    case "live" => s"https://microservicestraininggateway.api.bbci.co.uk"
  }
  ...
  object Whitelist {
    private val path: Option[String] = environment match {
      ...
      case "test" | "live" => Some(s"/usr/lib/microservicestraininggateway/conf/whitelist/$environment.txt")
    }
