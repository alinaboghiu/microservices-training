...
object Config {
  ...
  private val configFile = Try(io.Source.fromFile("/etc/microservicestrainingquery/microservicestrainingquery.json").mkString) getOrElse "{}"
  ...
  val host = environment match {
    case "dev" | "mgmt" => "http://localhost:8089"
    case "int" | "test" => s"https://microservicestrainingquery.$environment.api.bbci.co.uk"
    case "live" => s"https://microservicestrainingquery.api.bbci.co.uk"
  }
  ...
  object Whitelist {
    private val path: Option[String] = environment match {
      ...
      case "test" | "live" => Some(s"/usr/lib/microservicestrainingquery/conf/whitelist/$environment.txt")
    }
