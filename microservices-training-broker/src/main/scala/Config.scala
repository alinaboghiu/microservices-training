...
object Config {
  ...
  private val configFile = Try(io.Source.fromFile("/etc/microservicestrainingbroker/microservicestrainingbroker.json").mkString) getOrElse "{}"
  ...
  val host = environment match {
    case "dev" | "mgmt" => "http://localhost:8086"
    case "int" | "test" => s"https://microservicestrainingbroker.$environment.api.bbci.co.uk"
    case "live" => s"https://microservicestrainingbroker.api.bbci.co.uk"
  }
  ...
  object Whitelist {
    private val path: Option[String] = environment match {
      ...
      case "test" | "live" => Some(s"/usr/lib/microservicestrainingbroker/conf/whitelist/$environment.txt")
    }
